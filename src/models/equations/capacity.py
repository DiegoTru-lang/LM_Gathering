...
def capacity_constraints(m: Container) -> list[Equation]:
    ...
    pipelineSQ_capacity = Equation(m, "pipelineSQ_capacity", domain=[n, nn, d, t, c], description="A pipeline must be installed to transport squared flow")
    pipelineSQ_capacity[n,nn,d,t,c].where[arcs[n, nn] & tp[t]] = q_interSQ[n,nn,d,t,c] <= (maxFlow[c]**2)/1e6*Sum(tt.where[(Ord(tt) <= Ord(t)) & tp[tt]], x_bar[n,nn,d,tt])
    ...
    return [facility_capacity, pipeline_capacity, pipelineSQ_capacity, unique_pipeline, define_acum_cap]
