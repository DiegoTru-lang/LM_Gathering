...
def lm_correlation(m: Container) -> list[Equation]:
    ...
    one_interval[j,pf,d,t].where[tp[t]] =  Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d,t]], x_pw[j,pf,d,t,pw]) == 1
    # one_interval[j,pf,d,t].where[tp[t]] =  Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d,t]], x_pw[j,pf,d,t,pw]) == Sum(tt.where[(Ord(tt) <= Ord(t)) & (tp[tt])], x_bar[j,pf,d,tt])
    ...
    compute_deltaPliqYLM[j,pf,d,t].where[tp[t]] =  deltaPliq[j,pf,d,t] == Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d,t]], deltaPliqYLM[j,pf,d,t,pw])
    ...
    deltaPliqYLM_bound[j,pf,d,t,pw].where[tp[t] & (Ord(pw) <= allowed_int[j,pf,d,t])] = deltaPliqYLM[j,pf,d,t,pw] <= 10*dp_max*x_pw[j,pf,d,t,pw]
    ...
    pressure_drop_from_j_to_pf[j,pf,d,t].where[tp[t]] =  deltaP[j,pf,t] >= Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d,t]], ylp[pw,j,pf,d,t]*deltaPliqYLM[j,pf,d,t,pw])
