...
def lm_correlation(m: Container) -> list[Equation]:
    ...
    one_interval[j,pf,d,t].where[selected_pipes[j,pf,d] & tp[t]] =  Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d]], x_pw[j,pf,d,t,pw]) == Sum(tt.where[Ord(tt) <= Ord(t)], x_bar[j,pf,d,tt])
    ...
    compute_deltaPliqYLM[j,pf,d,t].where[selected_pipes[j,pf,d] & tp[t]] =  deltaPliq[j,pf,d,t] == Sum(pw.where[Ord(pw) <= allowed_int[j,pf,d]], deltaPliqYLM[j,pf,d,t,pw])
