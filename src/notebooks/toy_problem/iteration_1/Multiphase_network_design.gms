...
Parameter maxFlow(c) "Maximum flow of component 'c' [mscf per day]";
...
positive Variable QinterSQ(n,nn,d,t,c) "Squared flow of component 'c' through pipeline segment between nodes 'n' and 'nn' of diameter 'd' during time period 't' [(mscf per day)**2]";
...
positive Variable vel_liq_sq(n,nn,d,t) "Squared liquid velocity between nodes 'n' and 'nn' during time period 't' assuming pipe diameter 'd' [m/s]";
...
Equation SQ_mass_balance_ij(i,t,c) "Mass balance for source-junction for squared flow";
Equation SQ_mass_balance_jpf(j,t,c) "Mass balance for junction-processing facility for squared flow";
...
Equation pipelineSQ_capacity(n,nn,d,t,c) "A pipeline must be installed to transport squared flow";
...
SQ_mass_balance_ij(i,t,c) $ (ord(t) >= st_time(i)) .. sum(tt $ (ord(tt) eq ord(t) + 1 - st_time(i)),power(Qprod(c,tt),2)) / 1000000.0 =e= sum((nn,d) $ (arcs(i,nn)),QinterSQ(i,nn,d,t,c));
SQ_mass_balance_jpf(j,t,c) .. sum((nn,d) $ (arcs(nn,j)),QinterSQ(nn,j,d,t,c)) =e= sum((nn,d) $ (arcs(j,nn)),QinterSQ(j,nn,d,t,c));
...
pipeline_capacity(n,nn,d,t,c) $ (arcs(n,nn) and tp(t)) .. Qinter(n,nn,d,t,c) =l= maxFlow(c) * sum(tt $ (ord(tt) <= ord(t) and tp(tt)),x_bar(n,nn,d,tt));
pipelineSQ_capacity(n,nn,d,t,c) $ (arcs(n,nn) and tp(t)) .. QinterSQ(n,nn,d,t,c) =l= power(maxFlow(c),2) / 1000000.0 * sum(tt $ (ord(tt) <= ord(t) and tp(tt)),x_bar(n,nn,d,tt));
...
compute_velLIQ(j,pf,d,t) $ (arcs(j,pf) and tp(t)) .. vel_liq_sq(j,pf,d,t) =e= (QinterSQ(j,pf,d,t,"oil") + QinterSQ(j,pf,d,t,"water")) * 0.158987 * 0.158987 / power(3.141592653589793 * power(diam(d) * 0.0254,2) / 4 * 24 * 3600,2);
compute_dpLIQ(j,pf,d,t) $ (arcs(j,pf) and tp(t)) .. deltaPliq(j,pf,d,t) * 1000.0 =e= dist(j,pf) * 1.60934 * (hffl / 2) * (rho_liq * vel_liq_sq(j,pf,d,t) * 1000000.0) / (diam(d) * 0.0254);
weymouth_correlation(j,pf,d,t) $ (sel_pipes(j,pf,d) and tp(t)) .. pressGAS(pf,t) * pressGAS(pf,t) * 1000000.0 =l= pressSQ(j,t) * 1000000.0 - power(QinterSQ(j,pf,d,t,"gas") * 1000000.0 * (28.316819907857067 * kw(j,pf,d)),2);
