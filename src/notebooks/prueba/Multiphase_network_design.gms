...
Parameter ixlm_ub(pw) "IXLM upper bound for LM interval 'pw' ";
Parameter ylp(pw) "YLM multiplier for liquid phase for LM interval 'pw' ";
...
binary Variable x_pw(j,pf,t,pw) "Equals 1 if connection 'j' to 'pf' lies between ixlm interval of piecewise linearization 'pw' ";
...
positive Variable deltaPliqYLM(n,nn,t,pw) "Intermediate variable equal to pressure drop 'liquid-only' between nodes 'n' and 'nn' during time period 't' [MPa]";
...
Equation one_interval(j,pf,t) "Each connection (j, pf) must lie between a single 'pw' at each time period 't' ";
Equation ixlm_interval(j,pf,t,pw) "Impose interval for LM correlation between junction 'j' and processing facility 'pf' during time period 't' ";
Equation compute_deltaPliqYLM(j,pf,t) "Intermediate variable definition";
Equation deltaPliqYLM_bound(j,pf,t,pw) "Fix to 0 intermediate variable if IXLM does not lie between interval 'pw' ";
Equation pressure_drop_from_j_to_pf(j,pf,t) "Compute pressure drop from junction 'j' to processing facility 'pf' ";
...
one_interval(j,pf,t) $ (arcs(j,pf) and tp(t)) .. sum(pw,x_pw(j,pf,t,pw)) =e= 1;
ixlm_interval(j,pf,t,pw) $ (arcs(j,pf) and tp(t)) .. deltaPgas(j,pf,t) =l= ((ixlm_ub(pw) * sum(d,deltaPliq(j,pf,d,t))) + (dp_max * (1 - x_pw(j,pf,t,pw))));
compute_deltaPliqYLM(j,pf,t) $ (arcs(j,pf) and tp(t)) .. sum(d,deltaPliq(j,pf,d,t)) =e= sum(pw,deltaPliqYLM(j,pf,t,pw));
deltaPliqYLM_bound(j,pf,t,pw) $ (arcs(j,pf) and tp(t)) .. deltaPliqYLM(j,pf,t,pw) =l= (dp_max * x_pw(j,pf,t,pw));
pressure_drop_from_j_to_pf(j,pf,t) $ (arcs(j,pf) and tp(t)) .. deltaP(j,pf,t) =g= sum(pw,(ylp(pw) * deltaPliqYLM(j,pf,t,pw)));
