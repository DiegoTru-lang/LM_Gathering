...
$onMultiR
$gdxLoadAll c:\Users\Diego\Desktop\ExxonMobil\LM_Gathering\src\notebooks\prueba\iteration_10\Multiphase_network_design_data.gdx
$offMulti
...
one_interval(j,pf,d,t) $ (sel_pipes(j,pf,d) and tp(t)) .. sum(pw $ (ord(pw) <= allowed_int(j,pf,d,t)),x_pw(j,pf,d,t,pw)) =e= sum(tt $ (ord(tt) <= ord(t)),x_bar(j,pf,d,tt));
ixlm_interval(j,pf,d,t,pw) $ (sel_pipes(j,pf,d) and tp(t) and ord(pw) <= allowed_int(j,pf,d,t)) .. deltaPgas(j,pf,t) =l= (ixlm_ub(pw,j,pf,d,t) - 1000.0) * deltaPliq(j,pf,d,t) + dp_max * (1 - x_pw(j,pf,d,t,pw));
compute_deltaPliqYLM(j,pf,d,t) $ (sel_pipes(j,pf,d) and tp(t)) .. deltaPliq(j,pf,d,t) =e= sum(pw $ (ord(pw) <= allowed_int(j,pf,d,t)),deltaPliqYLM(j,pf,d,t,pw));
deltaPliqYLM_bound(j,pf,d,t,pw) $ (sel_pipes(j,pf,d) and tp(t) and ord(pw) <= allowed_int(j,pf,d,t)) .. deltaPliqYLM(j,pf,d,t,pw) =l= dp_max * x_pw(j,pf,d,t,pw);
pressure_drop_from_j_to_pf(j,pf,d,t) $ (sel_pipes(j,pf,d) and tp(t)) .. deltaP(j,pf,t) =g= sum(pw $ (ord(pw) <= allowed_int(j,pf,d,t)),ylp(pw,j,pf,d,t) * deltaPliqYLM(j,pf,d,t,pw));
